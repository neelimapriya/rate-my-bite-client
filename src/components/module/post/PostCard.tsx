    ...
    <Card key={spot.id} className="overflow-hidden py-0 h-full flex flex-col">
      ...
      <CardHeader className="">
        <div className="flex justify-between items-start">
          ...
          <span className="text-sm whitespace-nowrap font-medium text-gray-500">
            {spot.price} TK
          </span>
        </div>
      ...
      <CardFooter className="pt-0 pb-3 flex justify-between">
        <div className="flex items-center">
          ...
          <span className="ml-2 text-sm font-medium">
            {Math.ceil(
              spot?.ratings!.reduce((a, b) => a + Number(b.value), 0)
            ) || 10}
          </span>
        </div>
        <Button
          variant="ghost"
          className="hover:bg-primary hover:text-white transction-colors duration-500"
          size="sm"
          asChild
        >
